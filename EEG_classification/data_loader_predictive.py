...
records = []
with open("D:\deep/data/RECORDS") as file:
...
seizure_records = []
with open("D:\deep/data/RECORDS-WITH-SEIZURES") as file:
    lines = file.readlines()
...
for file_idx in range(1, 25):
    file_name = f"D:\deep/data/chb{str(file_idx) if file_idx >= 10 else ('0' + str(file_idx))}/chb{str(file_idx) if file_idx >= 10 else ('0' + str(file_idx))}-summary.txt"
...
# (count * 100, min((count + 1) * 100, len(records))):
for i in range(0, len(records)):
    ...
    record = records[i]
    with pyedflib.EdfReader("D:\deep\data/" + record) as f:
        ...
        sigbufs = None
        sigbufs_reshaped = None

        del sigbufs
        del sigbufs_reshaped
        del nonseizure_count
        f._close()
        del f
        gc.collect()
...
np.savez("predictive_signals.npz", signals=bufs, labels=bufs_labels)
